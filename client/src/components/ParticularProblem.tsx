...
// const [analyzeSaved,setAnayzedSaved]=useState<AnalyzeData[]>([]);
  ...
  const analyzeComp=async()=>{
    ...
    try{
      ...
      if(response.data.message=== 'Time Complexity analyzed successfully'){
...
const analyzeSpace=async()=>{
 ...
 const send={userCode,language};
    try{
      const response=await axios.post("http://localhost:5000/api/submit/analyzeSpaceComplexityOfCode",send,{withCredentials:true});
      if(response.data.message=== 'Space Complexity analyzed successfully'){
        const data=response.data.data;
        setIsblurred(true);
         Swal.fire({
        icon: "success",
        title: `Space Complexity ${data.space}`,
        text:`Explanation:${data.explanation}`,
    background: "#0b1b2b",
    color: "#e2e8f0",
      }).then(()=>{
      ...
      if(error.response?.data?.message=== 'provide proper detail'){
        Swal.fire({
          icon:"error",
          title:"space complexity option will be added soon",
           timer: 1000,
    showConfirmButton: false,
    background: "#0b1b2b",
    color: "#e2e8f0",
        ...
        Swal.fire({
          icon:"error",
          title:"Error analyze Complexity",
           timer: 1000,
    showConfirmButton: false,
    background: "#0b1b2b",
    color: "#e2e8f0",
  ...
  <span className={`pp-difficulty-text ${harsh?.difficulty?.toLowerCase()}`}>{harsh?.difficulty}</span>
        ...
        {analyze?(
          <>
          <button onClick={analyzeComp} className="analyze-btn">Analyze Time Complexity</button>
          <button onClick={analyzeSpace} className="analyze-btn">Analyze Space Complexity</button>
        </>
        ):
